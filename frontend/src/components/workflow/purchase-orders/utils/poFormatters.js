...
export const getPOStatusColor = (status) => {
  const statusColors = {
    'draft': 'bg-[#3A3A3C] text-[#EBEBF5] border-[#38383A]',
    'pending': 'bg-[#FF9F0A]/20 text-yellow-800 border-yellow-300',
    'approved': 'bg-[#30D158]/20 text-green-800 border-green-300',
    'rejected': 'bg-[#FF3B30]/20 text-red-800 border-red-300',
    'cancelled': 'bg-[#3A3A3C] text-[#8E8E93] border-[#38383A]',
    'completed': 'bg-[#0A84FF]/20 text-blue-800 border-blue-300'
  };
  
  return statusColors[status?.toLowerCase()] || 'bg-[#3A3A3C] text-[#EBEBF5] border-[#38383A]';
};
...
export const getProgressColor = (purchased, total) => {
  ...
  if (totalQty === 0) return 'bg-[#48484A]';
  
  const percentage = (purchasedQty / totalQty) * 100;
  
  if (percentage < 30) return 'bg-red-500';
  if (percentage < 70) return 'bg-yellow-500';
  if (percentage < 100) return 'bg-[#0A84FF]';
  return 'bg-green-500';
};
