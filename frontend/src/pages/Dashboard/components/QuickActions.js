...
const QuickLinks = () => {
  ...
  return (
    <div className="bg-[#2C2C2E] border border-[#38383A] p-5 rounded-xl hover:border-[#48484A] transition-colors">
      ...
      <div className="space-y-3">
        {links.map((link, index) => (
          <button 
            ...
            className="w-full flex items-center justify-between p-2.5 text-left border border-[#38383A] rounded-lg hover:bg-[#3A3A3C] hover:border-[#48484A] transition-colors duration-150 group"
          >
            <div className="flex items-center">
              <link.icon className={`h-5 w-5 ${link.colorClass} mr-3 group-hover:scale-110 transition-transform`} />
              <span className="font-medium text-white text-sm">{link.text}</span>
            </div>
