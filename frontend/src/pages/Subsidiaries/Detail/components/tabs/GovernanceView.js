  ...
  return (
    <div className="space-y-6">
      ...
      {subsidiary.boardOfDirectors && subsidiary.boardOfDirectors.length > 0 ? (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start space-x-4">
                    <div className="w-12 h-12 bg-[#0ea5e9]/20 rounded-full flex items-center justify-center flex-shrink-0">
                      <User className="h-6 w-6 text-[#0ea5e9]" />
                    </div>
                    <div>
                      <h4 className="text-white font-semibold text-base">{director.name || '-'}</h4>
                      <p className="text-white/60 text-sm mt-1">{director.position || '-'}</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 pt-4 border-t border-white/5">
                  <div>
                    <label className="block text-xs text-white/50 mb-1 flex items-center">
        ...
        <div className="text-center py-12 rounded-2xl border border-white/5 bg-white/5">
          <Users className="h-12 w-12 text-white/30 mx-auto mb-4" />
          <p className="text-white/60">Belum ada data dewan direksi</p>
        </div>
      )}
        ...
        <div className="rounded-2xl border border-white/5 bg-white/5 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.entries(subsidiary.profileInfo.socialMedia).map(([platform, url]) => (
              <div key={platform}>
                <label className="block text-xs uppercase tracking-[0.15em] text-white/50 mb-2 capitalize">
                  {platform}
                </label>
                <a 
                  href={url}
                  ...
                  className="text-[#0ea5e9] text-sm hover:text-[#38bdf8] transition-colors break-all"
                >
                  {url}
                </a>
              </div>
              ...
              >
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 bg-[#0ea5e9]/20 rounded-xl flex items-center justify-center flex-shrink-0">
                    <svg className="h-5 w-5 text-[#0ea5e9]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <div>
                    ...
                    {attachment.type && (
                      <p className="text-white/50 text-xs mt-0.5">{attachment.type}</p>
                    )}
                  </div>
                </div>
                {attachment.url && (
                  <a 
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-[#0ea5e9] text-sm hover:text-[#38bdf8] transition-colors"
                  >
                    Download
                  </a>
                )}
